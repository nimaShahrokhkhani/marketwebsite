...
class BrandManager extends Component {
    ...
    updateState = (item) => {
        const itemIndex = this.state.items.findIndex(data => data.name === item.name)
        const newArray = [
    ...
    deleteItemFromState = (name) => {
        Services.deleteBrand({name: name}).then((response) => {
            ...
            <>
                {isDone && !isLoading ?
                    <Container className="AppContainer">
                        <Row>
                            <Col>
                                <h1 style={{margin: "20px 0", display: 'flex', fontSize: 30}}>Brands</h1>
                            </Col>
                        </Row>
                        <Row>
                            <Col>
